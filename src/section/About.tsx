...
export default function About() {
  return (
    <div className={tw`h-full flex justify-between items-center px-[10%]`}>
      <div className={tw`flex flex-col items-start`}>
        <h2 className={tw(title, `mb-6`)}>HENDRIX</h2>
        <div className={tw`flex flex-col gap-8`}>
          ...
          <div className={tw`flex flex-col gap-4`}>
            {MY_HISTORY.map((el, index) => (
              <div key={index} className={tw`flex justify-start items-center gap-4`}>
      ...
      <div className={tw`h-full border-x-1 flex items-center`}>
        <div
          className={tw(
            `h-[442px] w-[377px] bg-contain bg-no-repeat border-y-1`,
            css`
